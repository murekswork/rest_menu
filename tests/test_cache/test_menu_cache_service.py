...
class TestMenuCacheServiceCreate(TestMenuCacheService):
    ...
    @pytest.mark.asyncio
    async def test_when_create_menu_correct_cache_appears(
            self,
            client: AsyncClient,
        ...
        cached_menu = json.loads(
            await redis_client.get(str(response.json()['id'])))
    ...
    @pytest.mark.asyncio
    async def test_when_create_menu_menus_list_cache_updates(
            self,
            client: AsyncClient,
            redis_client: aioredis.Redis,
            clean_tables):
...
class TestMenuCacheServiceDelete(TestMenuCacheService):
    ...
    @pytest.mark.asyncio
    async def test_when_delete_menu_then_menu_cache_deletes(
            self,
            client: AsyncClient,
            redis_client: aioredis.Redis,
            clean_tables):
        ...
        await client.delete(
            await reverse('menu-delete', target_menu_id=response['id']))
        new_cache = (await redis_client.get(str(response['id'])))
        assert old_cache != new_cache
        assert new_cache is None

    @pytest.mark.asyncio
    async def test_when_delete_menu_then_submenu_cache_deletes(
            self,
            client: AsyncClient,
            redis_client: aioredis.Redis,
            clean_tables):
    ...
    @pytest.mark.asyncio
    async def test_when_delete_menu_then_dish_cache_deletes(
            self,
            client: AsyncClient,
            redis_client: aioredis.Redis,
            clean_tables):
    ...
    @pytest.mark.asyncio
    async def test_when_delete_menu_then_submenus_list_cache_deletes(
            self,
            client: AsyncClient,
            redis_client: aioredis.Redis,
            clean_tables):
        ...
        submenu = await client.post(
            ...
            json={'title': 'title', 'description': 'description'})
        assert submenu.status_code == 201
        response = await client.get(await reverse('submenu-read-list',
                                                  target_menu_id=menu.json()[
                                                      'id']))
        assert response.status_code == 200
        submenu_list_cache = await redis_client.get(
            f'{menu.json()["id"]}_submenus')
        assert submenu_list_cache is not None
        delete = await client.delete(
            await reverse('menu-delete', target_menu_id=menu.json()['id']))
        assert delete.status_code == 200
        new_submenu_list_cache = await redis_client.get(
            f'{menu.json()["id"]}_submenus')
        assert new_submenu_list_cache != submenu_list_cache
        assert new_submenu_list_cache is None

    @pytest.mark.asyncio
    async def test_when_delete_menu_then_submenu_dish_list_deleted(
            self,
            client: AsyncClient,
            redis_client: aioredis.Redis,
            clean_tables):
        ...
        list_dishes_response = await client.get(await reverse('dish-list',
                                                              target_menu_id=menu.json()[
                                                                  'id'],
                                                              target_submenu_id=submenu.json()[
                                                                  'id']))
        assert list_dishes_response.status_code == 200
        list_dishes_cache = await redis_client.get(
            f"{submenu.json()['id']}_dishes")
        assert list_dishes_cache is not None
        menu_delete = await client.delete(
            await reverse('menu-delete', target_menu_id=menu.json()['id']))
        assert menu_delete.status_code == 200
        new_dishes_cache = await redis_client.get(
            f"{submenu.json()['id']}_dishes")
...
class TestMenuCacheServiceUpdate(TestMenuCacheService):
    ...
    @pytest.mark.asyncio
    async def test_when_update_menu_then_menu_cache_updates_correct(
            self,
            client: AsyncClient,
            redis_client: aioredis.Redis,
            clean_tables):
        ...
        update_menu = await client.patch(
            await reverse('menu-patch', target_menu_id=menu.json()['id']),
    ...
    @pytest.mark.asyncio
    async def test_when_update_menu_then_menu_counts_cache_deletes(
            self,
            client: AsyncClient,
            redis_client: aioredis.Redis,
            clean_tables):
        ...
        menu_counts_cache = await redis_client.get(
        ...
        update_menu = await client.patch(
            ...
            json={'title': 'new-title', 'description': 'new-description'})
        assert update_menu.status_code == 200
        menu_new_counts_cache = await redis_client.get(
            f"{menu.json()['id']}_counts")
