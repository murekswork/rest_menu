...
class TestSubmenuCacheServiceCreate(TestSubmenuCacheService):
    ...
    async def test_when_create_submenu_then_menu_cache_deletes(self,
        ...
        response_menu_submenus = await client.get(await reverse('submenu-read-list', target_menu_id=menu.json()['id']))
        assert response_menu_submenus.status_code == 200

        response_menu_counts = await client.get(await (reverse('menu-read-counts', target_menu_id=menu.json()['id'])))
