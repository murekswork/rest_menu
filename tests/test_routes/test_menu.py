...
class TestMenu:
    ...
    @pytest.mark.asyncio
    async def test_read_empty_menus(
            self,
            client: AsyncClient,
            clean_tables,
            clean_cache):
    ...
    async def test_read_menu(self, client: AsyncClient):
        ...
        response = await client.get(
            await reverse('menu-read', target_menu_id=menu_id))
        assert response.status_code == 200
    ...
    async def test_delete_menu(self, client: AsyncClient, clean_tables):
        ...
        response = await client.delete(
            await reverse('menu-delete', target_menu_id=menu_id))
        assert response.status_code == 200
    ...
    async def test_update_menu(self, client: AsyncClient, clean_tables):
        ...
        new_response = await client.patch(
            await reverse('menu-patch', target_menu_id=old_menu['id']),
