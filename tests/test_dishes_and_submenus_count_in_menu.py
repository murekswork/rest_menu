...
class TestDishesAndSubmenusCountInMenu:
    ...
    # according test requirements creates menu and its submenu, then test
    # creates first dish
    @pytest.mark.asyncio
    async def test_create_first_dish(self, client: AsyncClient, clean_tables,
    ...
    @pytest.mark.asyncio
    async def test_create_second_dish(self, client: AsyncClient, get_menu,
                                      get_submenu):
        response = await client.post(
            f'/api/v1/menus/{get_menu}/submenus/{get_submenu}/dishes',
    ...
    # Test that menu counts route works properly and return 1 submenu and 2
    # dishes in created menu
    @pytest.mark.asyncio
    ...
    @pytest.mark.asyncio
    async def test_submenu_has_count(self, client: AsyncClient, get_menu,
                                     get_submenu):
        response = await client.get(
            f'/api/v1/menus/{get_menu}/submenus/{get_submenu}')
        assert response.status_code == 200
        assert response.json()['dishes_count'] == 2

    # Deletes submenu
    @pytest.mark.asyncio
    async def test_delete_submenu(self, client: AsyncClient, get_menu,
                                  get_submenu):
    ...
    @pytest.mark.asyncio
    async def test_menu_after_submenu_delete(self, client: AsyncClient,
                                             get_menu):
        response = await client.get(f'/api/v1/menus/{get_menu}/counts')
        assert response.status_code == 200
